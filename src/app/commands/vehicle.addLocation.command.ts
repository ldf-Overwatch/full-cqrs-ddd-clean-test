...
export class VehicleAddLocationCommands extends TCommand {
  private vehicle: Query<
    Document<unknown, UnmarshalledVehicle> & UnmarshalledVehicle & { _id: Types.ObjectId },
    Document<unknown, UnmarshalledVehicle> & UnmarshalledVehicle & { _id: Types.ObjectId },
    UnmarshalledVehicle
  >;
  private location: Query<
    Document<unknown, UnmarshalledLocation> & UnmarshalledLocation & { _id: Types.ObjectId },
    Document<unknown, UnmarshalledLocation> & UnmarshalledLocation & { _id: Types.ObjectId },
    UnmarshalledLocation
  >;
  ...
  public async execute() {
    ...
    if ((await vehicle)?.location?.id === (await location)?.id) {
      ...
      const newVehicle = await VehicleRepository.findOneAndUpdate(
        ...
        { $set: { location: (await location)._id } },
        { new: true },
      );
